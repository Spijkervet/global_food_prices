...
class clustering():
    ...
    def ClusterCompare(self):
        """
        kijkt of clusters hetzelfde zijn en voegt ze samen.

        algortime is als volgt:
        bereken de std.

        """

        ChangedClusterAmount = False  

        # bereken de std
        group_std = np.empty((self.NGroups, self.shape[1]), dtype=np.float32)
        for cluster in range(self.NGroups): 
            Cluster = self.data[np.where(self.data[:, -1] == cluster)]
            group_std[cluster,:] = np.nanstd(Cluster[:, :-1], axis=0) 
        
        CompareDict = {}
        for cluster in range(self.NGroups): 
            CombineLst = []
            for nextcluster in range(cluster + 1, self.NGroups):
                DiffVectorOfAvg = self.NewGroupAvg[cluster] - self.NewGroupAvg[nextcluster] #the difference between the clusters
                DiffVectorOfAvgLength = np.sqrt(np.sum(DiffVectorOfAvg ** 2))
                DiffUnitVectorOfAvg = DiffVectorOfAvg / DiffVectorOfAvgLength #the difference between the cluster of length 1
                StdAlongUnitVectorlength0 = np.sqrt(np.sum(np.dot(group_std[cluster], DiffUnitVectorOfAvg) ** 2))
                StdAlongUnitVectorlength1 = np.sqrt(np.sum(np.dot(group_std[nextcluster], DiffUnitVectorOfAvg) ** 2))
        ...
        #             ChangedClusterAmount = True #if CompareDict[cluster] is not empty at least two clusters are combined
        ...
        # if ChangedClusterAmount: #Update the new clusters with new cluster numbers (to make them have the range(0,self.NumberOfClusters again)
        #     newcluster = 0
